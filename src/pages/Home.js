...
import {Container, Row, Col, Button, Card, Dropdown, Form,FormCheck} from "react-bootstrap";
...
// _____________________________login stuff & scraping async functions______________________________//
    ...
    // Notifications/Toast here
    const toastId = React.useRef(null)

    const initialToast = (message) => {
        toastId.current = toast(message, {type: toast.TYPE.INFO, autoClose: 20000})
    }
...
//____________________API functions here_______________________
    ...
    async function login(user_in,pass){
        ...
        try{
            const message = await axios('https://gscalapi.herokuapp.com' +'/login?email=' + user + '&pass=' + password);
    ...
    async function getName(){
        //
        const data = await axios('http://localhost:3001/get_name')
        const name = await parsedName(data['data'])
        // console.log(name)

    }

    async function parsedName(data){
            ...
            <div>
                <form className={"m-5"}>
                    ...
                    <div className = 'form-check'>
                        <input class = 'form-check-input' type='radio' name='DuoAuth' id='inlineCheckbox1' onChange = {(e) => {changeMethod2(true);changeMethod1(false)}}></input>
                        <label class="form-check-label" for="flexRadioCheckedDisabled">Regular Login</label>

                    </div>

                    <div class = 'form-check'>
                        <input class = 'form-check-input' type='radio' name='DuoAuth' id='inlineCheckbox1' onChange={(e) => {changeMethod1(true);changeMethod2(false)}}></input>
                        <label class="form-check-label" for="flexRadioCheckedDisabled">Duo Authentication</label>
                    </div>

                    <button type="submit" className="mb-5 btn btn-primary w-25" onClick={(e)=>loginMethod(email,pass)} >Submit</button>
